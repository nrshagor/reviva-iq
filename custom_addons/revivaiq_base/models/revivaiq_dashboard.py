...
class RevivaIQDashboard(models.Model):
    ...
    dead_stock_days_threshold = fields.Integer(string="Dead Stock Threshold (Days)", default=60, required=True)
    dead_stock_max_batch_limit = fields.Integer(string="Maximum Product Batch Limit", default=500, required=True)
    dead_stock_min_quantity = fields.Float(string="Minimum Stock Quantity", default=1.0)

    customer_inactivity_days_threshold = fields.Integer(string="Customer Inactivity Threshold (Days)", default=90, required=True)
    customer_batch_limit = fields.Integer(string="Maximum Customer Batch Limit", default=500, required=True)
    ...
    def _compute_dashboard_summary(self):
        for record in self:
            ...
            record.high_score_customer_count = len(customers.filtered(lambda c: c.recovery_score >= 80))
            record.customer_recovery_value = sum(customers.mapped("total_revenue"))

            record.latest_snapshot_count = len(latest_snapshots)
            record.latest_total_revenue_risk = sum(latest_snapshots.mapped("total_revenue_risk"))
